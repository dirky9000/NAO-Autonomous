...
// Sensor pins
#define trigPin1 30 // Pin for sensor 1 trigger
#define echoPin1 31 // Pin for sensor 1 echo
...
void loop() {
  ...
  if (distance1 < 39 && !isTurningLeft && !hasTurnedLeft) {
  ...
  // Check if the wheel needs to turn and hasn't turned yet
  if (distance2 > 50 && !isTurningRight && !hasTurnedRight) {
  ...
  // Check if the wheel needs to go back to the middle and has turned left
  if (distance2 < 50 && hasTurnedRight) {
    isTurningRight = true;  // Set the flag to indicate turning
